...
async function getClientRating(req, res) {
    try {
        ...
        const existingRating = await Rating.findOne({ event_id }).exec();

        if (existingRating) {
            const clientStar = existingRating.user.find((user) => user.client_id.toString() === client_id.toString());
            let star = 0
            if (clientStar) {
                star = clientStar.star
            }
